...
class requestResponseHandler:
    ...
    def processMessage(self):
        try:
            ...
            if "Accept-Encoding:" in mappedMsg:
                encoding =  mappedMsg.rpartition("Accept-Encoding:")[-1].strip().split(",")
                if 'gzip' in encoding:
                    self.gzipEncode = True
            #print mappedMsg
            ...
            if (reqMethod == 'GET'):
                if ("/exec/" in type):
                    ...
                    if self.gzipEncode:
                        responseHeaders = self.generateResponseHeaders(200,"gzip")
                        out = StringIO.StringIO()
                        with gzip.GzipFile(fileobj=out, mode="w") as f:
                            f.write(body)
                        responseBody = out.getvalue()
                        out.close()
                    try:
                        print "###########"
                        print process.pid
